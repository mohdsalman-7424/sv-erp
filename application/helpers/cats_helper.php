...
//get data from inch order contract table//

if (!function_exists('find_gst_data')) {
    function find_gst_data($id)
    {
        $CI = &get_instance();
        $CI->db->where("FIND_IN_SET($id,product_id)!=0");
        $CI->db->select("*");
        //$CI->db->order_by("name","asc");
        $result = $CI->db->get("inch_order_contract");
        if ($result->num_rows > 0) {
            return $result->row();
        } else {
            return false;
...
if (!function_exists('get_contact_list')) {
    function get_contact_list($id=NULL, $compID=NULL, $columns=NULL)
    {
        $CI = &get_instance();
        ...
        {
            $CI->db->select("id, name, primary_phone");
        }
        ...
        {
            $CI->db->where("company_id",$compID);
        }
        $CI->db->where("status","1");
        $result = $CI->db->get("companies_contact");
        if ($result->num_rows() > 0) {
            return $result->result();
        } else {
            return FALSE;
...
if (!function_exists('get_all_employee')) {
...
//get box data from order  boxes
if (!function_exists('get_box_data')) {
    function get_box_data($box_id,$order_id,$product_id)
    {
        $product_id = explode(',', $product_id);
        $CI = &get_instance();
        $CI->db->select("ep.*, um.name as unit_name, hsn.name as hsn_name,hsn.gst,hsn.hsn_no,hsn.customs_duty,sc.name as supplier_name, sv.name as supplier_type_name, concat(u.first_name,' ',u.last_name) as added_by_name,ps.name as product_status,obd.packed_qty as main_packed_qty",FALSE);
        $CI->db->where_in('ep.form_id',$product_id);
        $CI->db->where('ep.order_id',$order_id);
        $CI->db->where('obd.box_id',$box_id);
        $CI->db->join(TBL_PREFIX.'unit_master as um','um.form_id=ep.uom','LEFT');
        $CI->db->join(TBL_PREFIX.'hsn_code_master as hsn','hsn.form_id=ep.hsn_code','LEFT');
        $CI->db->join('supplier_vendor as sv','sv.id=ep.supplier_type','LEFT');
        $CI->db->join('supplier_china as sc','sc.id=ep.supplier','LEFT');
        $CI->db->join('users as u','u.id=ep.added_by','LEFT');
        $CI->db->join(TBL_PREFIX.'product_status as ps','ps.form_id=ep.product_status','LEFT');
        $CI->db->join(TBL_PREFIX.'order_box_details as obd','obd.product_id=ep.form_id','LEFT');
        $CI->db->order_by('ep.product_box_id',ASC);
        //$CI->db->group_by('obd.product_id');
        if($po_nos)
        $CI->db->order_by('ep.'.PRIMARY_KEY, 'ASC');
        $res = $CI->db->get('inch_order_product as ep')->result();
        return $res;
        
   }
}

//get Packing list box data from order  boxes
if (!function_exists('get_packing_box_detail')) {
    function get_packing_box_detail($order_id,$invoice_id,$product_id,$id)
    {
       $product_id = explode(',', $product_id);
        $CI = &get_instance();
        $CI->db->select("ep.*, um.name as unit_name, hsn.name as hsn_name,hsn.gst,hsn.hsn_no,hsn.customs_duty,sc.name as supplier_name, sv.name as supplier_type_name, concat(u.first_name,' ',u.last_name) as added_by_name,ps.name as product_status,obd.packed_qty as main_packed_qty",FALSE);
        $CI->db->where_in('ep.form_id',$product_id);
        $CI->db->where('ep.order_id',$order_id);
        $CI->db->where('obd.invoice_id',$invoice_id);
        $CI->db->where('obd.box_id',$id);
        $CI->db->join(TBL_PREFIX.'unit_master as um','um.form_id=ep.uom','LEFT');
        $CI->db->join(TBL_PREFIX.'hsn_code_master as hsn','hsn.form_id=ep.hsn_code','LEFT');
        $CI->db->join('supplier_vendor as sv','sv.id=ep.supplier_type','LEFT');
        $CI->db->join('supplier_china as sc','sc.id=ep.supplier','LEFT');
...
//get invoice packed qty order  boxes
if (!function_exists('get_invoice_packed_qty')) {
    function get_invoice_packed_qty($product_id,$order_id)
    {
        $CI = &get_instance();
        $CI->db->select("SUM(packed_qty) as total_packed_invoice_qty",FALSE);
        $CI->db->where('order_id',$order_id);
        $CI->db->where('product_id',$product_id);
        $CI->db->group_by('product_id');
        $res = $CI->db->get('inch_order_invoice_details')->row();
        return $res;
...
/**
 * All Company from Supplier China
 *
 * 
 *
 * @access    public
...
if (!function_exists('get_contact_from_master_acord_company')) {
    function get_contact_from_master_acord_company($id)
    {
        $CI = &get_instance();
        $CI->db->where('supplier_id', $id);
        $CI->db->where('status', '1');
        $CI->db->order_by('name', 'ASC');
        $result =$CI->db->get('supplier_ind_contact');
        if ($result->num_rows > 0) {
            return $result->result();
        } else {
            return false;
...
if (!function_exists('currentuserinfo')) {
    function currentuserinfo()
    {
        ...
        return $CI->session->userdata("auth_user");
 ...
 */
if (!function_exists('set_flashdata_old')) {
    function set_flashdata_old($type, $msg) {
        $CI = &get_instance();
...
/**
 * Set Custom Flash Data
 *
 * Set Custom Flash Data in Session Flashdata
 *
 * @access    public
 * @param   String - Message type value(info,success,warning,error)
 * @param   String - Text Message
...
if (!function_exists('get_flashdata')) {
    function get_flashdata() {
        ...
        session_start(); 
        // pr($_SESSION);die;
        $success = $_SESSION['custom_msg_success'] ? $_SESSION['custom_msg_success'] : '';
        $error   = $_SESSION['custom_msg_error'] ? $_SESSION['custom_msg_error'] : '';
        $warning = $_SESSION['custom_msg_warning'] ? $_SESSION['custom_msg_warning'] : '';
        if ($success) {
        ...
        unset($_SESSION['custom_msg_success']);
        unset($_SESSION['custom_msg_error']);
        unset($_SESSION['custom_msg_warning']);
        return $msg;
    }
}
 ...
 * @return  String
 */
if (!function_exists('get_flashdata_old')) {
    function get_flashdata_old() {
        $CI = &get_instance();
...
function get_industry_type_filters(){
    ...
    function dashboard_permission_data()
    {
        $CI = &get_instance();
        $dashboar_permission = [];
        $permissions = [];
        ...
        $dashboar_permission['enquiry_status_india']           = isset($permissions['dashboard/enquiry_status_india']) && !empty($permissions['dashboard/enquiry_status_india'])?true:false;
        $dashboar_permission['enquiry_status_india_pcb']       = isset($permissions['dashboard/enquiry_status_india_pcb']) && !empty($permissions['dashboard/enquiry_status_india_pcb'])?true:false;
        $dashboar_permission['enquiry_status_india_governing'] = isset($permissions['dashboard/enquiry_status_india_governing']) && !empty($permissions['dashboard/enquiry_status_india_governing'])?true:false;

        $dashboar_permission['enquiry_status_china']           = isset($permissions['dashboard/enquiry_status_china']) && !empty($permissions['dashboard/enquiry_status_china'])?true:false;
        $dashboar_permission['enquiry_status_china_pcb']       = isset($permissions['dashboard/enquiry_status_china_pcb']) && !empty($permissions['dashboard/enquiry_status_china_pcb'])?true:false;
        $dashboar_permission['enquiry_status_china_governing'] = isset($permissions['dashboard/enquiry_status_china_governing']) && !empty($permissions['dashboard/enquiry_status_china_governing'])?true:false;


        $dashboar_permission['paymet_receival_detail']           = isset($permissions['dashboard/paymet_receival_detail']) && !empty($permissions['dashboard/paymet_receival_detail'])?true:false;
        $dashboar_permission['paymet_receival_detail_pcb']       = isset($permissions['dashboard/paymet_receival_detail_pcb']) && !empty($permissions['dashboard/paymet_receival_detail_pcb'])?true:false;
        $dashboar_permission['paymet_receival_detail_governing'] = isset($permissions['dashboard/paymet_receival_detail_governing']) && !empty($permissions['dashboard/paymet_receival_detail_governing'])?true:false;

        $dashboar_permission['billing_details']                = isset($permissions['dashboard/billing_details']) && !empty($permissions['dashboard/billing_details'])?true:false;
        $dashboar_permission['billing_details_pcb']            = isset($permissions['dashboard/billing_details_pcb']) && !empty($permissions['dashboard/billing_details_pcb'])?true:false;
        $dashboar_permission['billing_details_governing']      = isset($permissions['dashboard/billing_details_governing']) && !empty($permissions['dashboard/billing_details_governing'])?true:false;

        $dashboar_permission['insurance_balance_sales_spare']    = isset($permissions['dashboard/insurance_balance_sales_spare']) && !empty($permissions['dashboard/insurance_balance_sales_spare'])?true:false;
        $dashboar_permission['insurance_balance_sales_pcb']       = isset($permissions['dashboard/insurance_balance_sales_pcb']) && !empty($permissions['dashboard/insurance_balance_sales_pcb'])?true:false;
        $dashboar_permission['insurance_balance_sales_governing'] = isset($permissions['dashboard/insurance_balance_sales_governing']) && !empty($permissions['dashboard/insurance_balance_sales_governing'])?true:false;
        // pr($permissions['dashboard/enquiry_status_india']);die;
        return $dashboar_permission;
    }
    ...
    if (!function_exists('get_hsncode_from_master_china')) {
        function get_hsncode_from_master_china()
        {
            ...
            $result = $CI->db->get('inch_chsn_code_master');
            if ($result->num_rows > 0) {
                return $result->result();
            } else {
                return false;
    ...
    function uploadDoc($file_arr, $folder_doc = 'assets/uploads/temp_attachment/', $upload_file="upload_file") {
        $folder_doc  =   FCPATH.$folder_doc;
...
if (!function_exists('get_payment_details')) { 
    function get_payment_details($order_id=null) {
        $CI = &get_instance();
        $CI->db->select('*');
        $CI->db->from('inch_payment_details');
        $CI->db->where('order_id',$order_id);
        $query = $CI->db->get();
        if($query->num_rows()>0) {
            return $query->result();
...
if (!function_exists('get_product_sequence')) { 
    function get_product_sequence($ids=null) {
        $CI = &get_instance();
        $CI->db->select('product_sequence');
        $CI->db->from('inch_order_product');
        $CI->db->where_in('form_id',$ids);
        $CI->db->order_by('product_sequence','asc');
        $query = $CI->db->get();
        if($query->num_rows()>0){
            foreach($query->result() as $p)
            {
                $sequence[] = $p->product_sequence;
            }
            if($sequence)
            {
...
if (!function_exists('get_supplier_info')) { 
    function get_supplier_info($id=null,$order_id,$country) {
        $CI = &get_instance();
        $supplier  = "";
        if($country == '2')
        {
            $supplier  = "Chengdu Fuxingyuan International Trade Co. Ltd";
        }
        else
        {
            $CI->db->select('s.name supplier')->from('inch_order_contract ioc');
            $CI->db->join("supplier_ind s",'s.id=ioc.supplier','left');
            $CI->db->where("FIND_IN_SET($id,ioc.product_id)!=0");
            $CI->db->where("ioc.order_id",$order_id);
            $query = $CI->db->get();
            if($query->num_rows()>0)
            {
...
if (!function_exists('get_supplier_info_for_stock_list')) { 
    function get_supplier_info_for_stock_list($id=null,$type) {
        $CI = &get_instance();
        if($type == 1)
        {
            $CI->db->select('s.name supplier_name,op.supplier_invoice_no,op.supplier_date,op.currency,s.gst_no supplier_gst');
            $CI->db->from('inch_order_contract op');
            $CI->db->join('supplier_ind s','s.id=op.supplier');
            $CI->db->where("FIND_IN_SET($id,op.product_id) != 0");
            $query = $CI->db->get();
            if($query->num_rows()>0){
                return $query->row();
            }
        ...
        {
            // pr
            $CI->db->select('mlb.lot_id');
            $CI->db->from('inch_manage_lot_box_details mlb');
            $CI->db->where("mlb.product_id",$id);
            $query = $CI->db->get();
            if($query->num_rows()>0)
            {
                $lot_id = $query->row()->lot_id;
                $CI->db->select('mlb.invoice_no supplier_invoice_no,mlb.consignee_name,mlb.invoice_dated supplier_date,ci.boe_no supplier_gst,mlb.lot_no china_dispatch_no');
                $CI->db->from('inch_manage_lot mlb');
                $CI->db->join('inch_custom_inch ci','ci.lot_id=mlb.id');
                $CI->db->where("mlb.id",$lot_id);
                $query = $CI->db->get();
                if($query->num_rows()>0)
                {
                    // $CI->db->select('mlb.invoice_no,mlb.invoice_dated');
                    return $query->row();
                }
            }
        }
...
if (!function_exists('get_box_products')) { 
    function get_box_products($id=null,$lot_id) {
        $CI = &get_instance();
        $CI->db->select('ld.*,ld.packed_qty box_packed_qty,iop.*');
        $CI->db->from('inch_manage_lot_box_details ld');
        $CI->db->join('inch_order_product iop','iop.form_id=ld.product_id');
        $CI->db->where('ld.lot_id',$lot_id);
        $CI->db->where('ld.box_id',$id);
        $query = $CI->db->get();
        if($query->num_rows()>0){
            return $query->result();
        }
...
function get_product_status()
{
    ...
    $CI->db->where("status","1");
    // $CI->db->order_by("name","asc");
    $result = $CI->db->get("inch_product_status");
    if ($result->num_rows > 0) {
        return $result->result();
    } else {
        return false;
...
{
        $CI =& get_instance();
        $dbhostname = $CI->db->hostname;
        $dbusername = $CI->db->username;
        $dbpassword = $CI->db->password;
        $dbdatabase = $CI->db->database;
        $host = $dbhostname;
        $username = $dbusername;
        $password = $dbpassword;
        ...
        if(!empty($sqlScript))
        {
            $folderName = './upload/downloads/';
                if (!is_dir($folderName)) {
                 mkdir($folderName, 0777, true);
            }
            // Save the SQL script to a backup file
            $backup_file_name = 'procloud_incherp_' . date('Y-m-d') . '.sql';
            $filepath = $folderName.$backup_file_name;
           ...
           //if(isset($_GET['download']) && !empty($_GET['download'])){
            $file_url = './upload/downloads/' . $backup_file_name;
