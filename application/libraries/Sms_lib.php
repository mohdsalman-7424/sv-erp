...
class Sms_lib {
    ...
    public function send_pos_order_confirm_sms($user_name, $number, $order_no, $amount = NULL, $order_type = 2,$smscontact=NULL,$smsemail=NULL) {
        $CI             = & get_instance();

        $CI->db->select('name, content');
        $CI->db->where('name', 'POS_TEMPLATE');
        $qry = $CI->db->get('tbl_sys_sms_template');
        if($qry->num_rows() > 0) {
            $data = $qry->result();
            $template = $data[0]->content;
            if(isset($template) && !empty($template))
            {
                $template = str_replace('$customer_name', ucwords(strtolower($user_name)), $template);
                $template = str_replace('$orderIds', $order_no, $template);
                $template = str_replace('$store_Number', $smscontact, $template);
                $template = str_replace('$store_mail',$smsemail, $template);
                $message = $template;
            }

        } else {
            $message = "Dear ".ucwords(strtolower($user_name)).", your order is completed and order id ".$order_no.". For any query call us ".$smscontact." or mail us on ".$smsemail." Raprocure.";
        }
        if ($number && $message) {
    ...
    public function send_order_confirm_sms($user_name, $number, $order_no, $amount = NULL, $order_type = 2,$smscontact=NULL,$smsemail=NULL)
    {
...

        {
            ...
            $template = str_replace('$totalamount', $totalamount, $template);


            $template = str_replace('$supportcontact',$smscontact, $template);
            $template = str_replace('$supportemail',$smsemail, $template);
...

        {
...

            {
                ...
                // $message = "Dear " . ucwords(strtolower($user_name)) . ',' . "Your order ". $order_no ." is confirmed your order will be ready for pickup by ".$timeslot." on ". $delivery_date . ". For any query call us on +91 9999999999 or mail us on support@Raprocure.com";

                $message = "Dear ". ucwords(strtolower($user_name)). ',' . " Your order ".$order_no." for order ".$delivery_date. " and delivery slot is: ".$timeslot.". For any query call us on ".$this->dbsettings->CONTACT_NUMBER." or mail us on ".$this->dbsettings->WEBSITE_EMAIL." Raprocure";
...

            {
                ...
                $message = "Dear " . ucwords(strtolower($user_name)) . ',' . "Your order ". $order_no ." is confirmed to be delivered on ". $delivery_date . " between ". $timeslot .". For any query call us on +91-9999999999 or mail us on support@Raprocure.com";
...

    {
...

        {
            ...
            $message = "Dear " . ucwords(strtolower($user_name)) . ',' . "Your order has been ".$type.". Order no is :" . $order_no . ' ' . ", For any query call us on +91-9999999999 or mail us on support@Raprocure.com or WhatsApp on +91-9999999999";
...

    {
...

        {
            ...
            $message = "Dear " . ucwords(strtolower($user_name)) . ',' . "Your order could not be placed. Order no is :" . $order_no . ' ' . ". Customer Support: +91-9999999999. Customer Support Email:support@Raprocure.com";
...
// pr($url);die();
...

    {
...

        {
            ...
            $template = str_replace('{#var#}', $otp, $template);
